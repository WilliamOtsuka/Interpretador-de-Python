...
int identificar_equacao(TpCode *atual, PilhaM *pilha) {
    ...
    if(strcmp(atual->token, ")") == 0) { // se for um parenteses
        ...
        if(!isOperation(atual->prox->token) || atual->prox != NULL) // se o próximo token não for uma operação ou termina com null
            return 0;
    }
    if(isOperation(atual->token)) { // se for uma operação
        if(strcmp(atual->prox->token, "(") != 0 || isOperation(atual->prox->token)) // se o próximo token não for um parenteses ou for uma operação
            return 0;
    }
        ...
        if(!isOperation(atual->prox->token) || atual->prox != NULL) // se o próximo token não for uma operação ou termina com null
            ...
            if(!isNumber(atual->token)) { // se não for um número
